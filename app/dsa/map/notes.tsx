        ...
        try {
            if (typeof window !== "undefined") {
                ...
                if (storedUser) {
                    ...
                    const response = await axios.post("/api/notes", { email: parsedUser.email, topic: 'Map', note: newNote });
    ...
    const deleteNote = async (title: string) => {
        try {
            if (typeof window !== "undefined") {
                ...
                if (storedUser) {
                    const parsedUser = JSON.parse(storedUser);
                    const response = await axios.put("/api/notes", { email: parsedUser.email, topic: 'Map', title: title });
                    getAllNotes();
                    toast.success(response.data.msg);
                }
            } else {
